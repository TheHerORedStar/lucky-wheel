...
const tzNames = (() => {
  ...
  const nonBlacklistedSuffixes = (zone) =>
    !(scrubbedSuffixes.indexOf(zone.split('/').slice(-1)[0]) >= 0);
  ...
  return Object.values(moment.tz._zones) // eslint-disable-line no-underscore-dangle
    ...
    .filter(
      (zone) =>
        onlyCanonicalZones(zone) && nonBlacklistedPrefixes(zone) && nonBlacklistedSuffixes(zone),
    );
})();
...
tzCities.forEach((city, index) => {
  ...
  tzMap.offset = moment().tz(tzName).format('Z');
  tzMap.id = tzName;
  tzMap.title = `(${tzMap.offset}) ${tzName}`;

  interimTzSet.add(tzMap);
});
// the interimTzSet is just used for the purpose of eliminating duplicates
const tzMaps = [...interimTzSet].sort((e1, e2) => {
  if (e1.offset < e2.offset) return 1;
  if (e1.offset > e2.offset) return -11;
  return 0;
});

export const timezones = moment.tz._zones;
export const timezoneNames = tzMaps.map((tz) => tz.id);
